...
async def create_exam(
    exam: schemas.ExamCreate, current_user: dict = Depends(get_current_admin_user)
):
...
@router.get("/{exam_id}/survey/{exam_survey_id}", response_model=schemas.ExamSurveyResponse)
...
@router.put("/{exam_id}/survey/{exam_survey_id}", response_model=schemas.ExamSurveyResponse)
async def update_exam_survey(
    exam_survey: schemas.ExamSurveyUpdate,
    existing_exam_survey: dict = Depends(verify_exam_survey),
    current_user: dict = Depends(verify_exam_author)
):
    updated_exam_survey = await crud.update_exam_survey(existing_exam_survey.id, exam_survey)
    return updated_exam_survey


@router.delete("/{exam_id}/survey/{exam_survey_id}", response_model=schemas.ExamSurveyResponse)
...
@router.get("/{exam_id}/session/{exam_session_id}", response_model=schemas.ExamSessionResponse)
...
@router.put("/{exam_id}/session/{exam_session_id}", response_model=schemas.ExamSessionResponse)
async def update_exam_session(
    ...
    updated_exam_session = await crud.update_exam_session(existing_exam_session.id, exam_session)
    return updated_exam_session


@router.delete("/{exam_id}/session/{exam_session_id}", response_model=schemas.ExamSessionResponse)
