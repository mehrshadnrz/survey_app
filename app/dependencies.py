...
async def verify_session_survey(
    survey_id: int,
    exam_session: dict = Depends(verify_exam_session),
    current_user: dict = Depends(check_user_access),
):
    survey = await crud.get_survey_with_questions(survey_id)
    if not survey:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Survey not found",
        )
    exist_survey_in_session = await crud.get_exam_survey_by_exam_and_survey(
        exam_id=exam_session.examId,
        survey_id=survey.id,
    )
    if not exist_survey_in_session:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Survey not found",
        )
...
async def verify_session_question(
    question_id: int,
    exam_session: dict = Depends(verify_exam_session),
    current_user: dict = Depends(check_user_access),
):
